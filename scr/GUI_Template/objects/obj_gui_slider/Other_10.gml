...
if (gui_active) {
	...
	if (mouse_check_button(mb_left) && _mouseOverGuiElement) {
		slider_value = ((_x - (x-sprite_width*0.5)) / sprite_width) * (slider_maxVal-slider_minVal) + slider_minVal;
	...
	if (lastValue != slider_value) {		
		...
		knobX = -sprite_width*0.5 + ((slider_value-slider_minVal) / (slider_maxVal-slider_minVal)) * sprite_width;
	}
}